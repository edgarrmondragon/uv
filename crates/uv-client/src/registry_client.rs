...
impl RegistryClientBuilder {
    ...
    pub fn build(self) -> RegistryClient {
        let client_raw = self.client.unwrap_or_else(|| {
            ...
            // `UV_REQUEST_TIMEOUT` is provided for backwards compatibility
            let default_timeout = 5 * 60;
            let timeout = env::var("UV_REQUEST_TIMEOUT").or_else(|_| env::var("UV_HTTP_TIMEOUT")).or_else(|_| env::var("HTTP_TIMEOUT")).and_then(|value| {
                value.parse::<u64>()
                    .or_else(|_| {
